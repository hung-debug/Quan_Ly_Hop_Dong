...
export class DetermineSignerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.datas.flagDigitalSign) {
      this.signTypeList = this.signTypeList.filter((p: any) => ![1, 5, 9].includes(p.id));
    }

    this.user = this.userService.getInforUser();

    this.isListSignNotPerson[0] = this.signTypeList.filter((p) => ![1, 5, 9].includes(p.id)); // person => sign all,
  ...
  getSignTypeList() {
    return this.signTypeList.filter((p) => ![1, 5, 9].includes(p.id))
  }
  ...
  selectWithOtp(e: any, data: any, type: any) { // sort ordering
    ...
    //Nếu là đối tác tổ chức
      ...
      if (this.getDataSignUSBToken(data).length == 0 && this.getDataSignEkyc(data).length == 0 && this.getDataSignHsm(data).length == 0 && this.getDataSignCert(data).length == 0 && this.getDataSignRemote(data).length == 0 && this.getDataSignCertOneTime(data).length == 0) {
        if (this.loadedMyPartner == false)
          data.card_id = "";
      }
  ...
  changeTypeSign(d: any,index: any,id?: any,role?: any) {
    ...
    if (role == 'sign_partner') {
      if (d.login_by == 'phone') {
        ...
        d.isListSignNotPersonPartner = this.signTypeList.filter((p) => ![1, 5, 9].includes(p.id));
      }
    } else if (role == 'signer') {
      if (d.login_by == 'phone') {
        this.isListSignNotPerson[index] = this.signTypeList.filter((p) => ![1, 2, 5, 7].includes(p.id));
      } else {
        this.isListSignNotPerson[index] = this.signTypeList.filter((p) => ![1, 5, 9].includes(p.id));
      }
  ...
  getDataSignCertOneTime(data: any) {
    return data.sign_type.filter((p: any) => p.id == 9);
  }
  ...
  validData() {
    ...
    for (let i = 0; i < dataArr.length; i++) {
      ...
      if (dataArr[i].sign_type.length == 0 && dataArr[i].role != 2) {
        ...
        let is_duplicate = [];
        //check chu ky so
        is_duplicate = dataArr[i].sign_type.filter((p: any) => p.id == 2 || p.id == 3 || p.id == 4 || p.id == 7 || p.id == 8);
    ...
    if (count == 0) {
      ...
      if (this.getCheckDuplicatePhone(onlyPartOrg, dataArr)) {
        this.getNotificationValid("SĐT tổ chức của tôi không được trùng nhau!");
        return false
      }
        ...
        for (let k = 0; k < isParterSort.length; k++) {
          ...
          if (dataArrPartner[j].type != 3) {
            ...
            if (isParterSort[k].sign_type.length == 0 && [3, 4].includes(isParterSort[k].role)) {
              ...
              //check chu ky so
              isPartnerOriganzationDuplicate = isParterSort[k].sign_type.filter((p: any) => p.id == 2 || p.id == 3 || p.id == 4 || p.id == 7 || p.id == 8);
            ...
            if (isParterSort[k].login_by == 'email' || (isParterSort[k].role == 1 && isParterSort[k].email)) {
              ...
              if (isParterSort[k].phone && !this.pattern.phone.test(isParterSort[k].phone?.trim())) {
                this.getNotificationValid("SĐT của " + this.getNameObjectValid(isParterSort[k].role) + "của đối tác không hợp lệ!")
                count++;
                break;
              }
            }
            ...
            if (!isParterSort[k].card_id?.trim() && (isParterSort[k].role == 3 || isParterSort[k].role == 4) && isParterSort[k].sign_type.filter((p: any) => p.id == 2 || p.id == 4 || p.id == 6 || p.id == 8).length > 0) {
              this.getNotificationValid("Vui lòng nhập MST/CMT/CCCD/Số hộ chiếu của" + this.getNameObjectValid(isParterSort[k].role) + "tổ chức của đối tác");
              count++;
              break;
            }
            ...
            isParterSort[k].sign_type.filter((p: any) => p.id == 8).length > 0) {
              this.getNotificationValid("Mã số thuế/CMT/CCCD/Số hộ chiếu của" + this.getNameObjectValid(isParterSort[k].role) + "tổ chức của đối tác không hợp lệ!");
              count++;
              break;
            }
          ...
          //Cá nhân
            ...
            if (isParterSort[k].sign_type.length == 0 && [3, 4].includes(isParterSort[k].role) && isParterSort[k].role == 3) {
              ...
              //check chu ky so
              isPartnerCaNhanDuplicate = isParterSort[k].sign_type.filter((p: any) => p.id == 2 || p.id == 3 || p.id == 4 || p.id == 7 || p.id == 8);
              ...
              if (!isParterSort[k].card_id?.trim() && (isParterSort[k].role == 3 || isParterSort[k].role == 4) && isParterSort[k].sign_type.filter((p: any) => p.id == 2 || p.id == 4 || p.id == 6).length > 0) {
                this.getNotificationValid("Vui lòng nhập MST/CMT/CCCD của" + this.getNameObjectValid(isParterSort[k].role) + "của đối tác cá nhân");
                count++;
                break;
              }
              
              if (!isParterSort[k].card_id?.trim() && (isParterSort[k].role == 3 || isParterSort[k].role == 4) && isParterSort[k].sign_type.filter((p: any) => p.id == 8).length > 0) {
                this.getNotificationValid("Vui lòng nhập MST/CMT/CCCD/Số hộ chiếu của" + this.getNameObjectValid(isParterSort[k].role) + "của đối tác cá nhân");
                count++;
                break;
              }
    ...
    if (count == 0) {
      ...
      if (isOrderingPerson_exception.length > 0 && arrCompareOrderingPartner.length > 0) {
        ...
        // let dataOrderingOrg = arrOrg[0]?.recipients.filter((item:any) => item.ordering < 2 && item.role !=2);
  ...
  addOriganzationSignature(index?: any) {
    let data_determine_add = [];
    data_determine_add = [...this.contractService.getDataDetermine()];
    let data_organization = data_determine_add.filter((p: any) => p.type == 1)[0];
    let data: any = (data_organization.recipients.filter((p: any) => p.role == 3))[0];

    this.isListSignNotPerson.push(this.signTypeList.filter((p: any) => ![1, 5, 9].includes(p.id)));
  ...
  dataParnterOrganization() {
    ...
    let isOrderingOrg = this.datas.is_determine_clone.filter((val: any) => val.type == 1);
    let isOrderingOrgPartner = this.datas.is_determine_clone.filter((val: any) => val.type == 2); // lấy mảng đối tác tổ chức
    let orderingPerson_exception = isOrderingPerson_exception[0]?.ordering; // lấy ra ordering của đối tác cá nhân

    let arrNoPartner = this.datas.is_determine_clone.filter((item: any) => item.type != 3) // lọc ra mảng chưa các phần tử không có đối tác cá nhân
  ...
  getListSignType(role?: any) {
    if (role == 'partner' || role == 'org') {
      return this.signTypeList.filter((p: any) => ![1, 5, 9].includes(p.id));
    } else {
  ...
  doTheSearch($event: Event, indexs: number, action: string): void {
    ...
    setTimeout(() => {
      this.contractService.getAllInfoUser(stringEmitted).subscribe((res) => {
        let arr_all = res;
        let data = environment.flag == 'KD' ? arr_all.map((p: any) => ({ name: p.name, email: p.email, phone: p.phone })) : arr_all.map((p: any) => ({ name: p.name, email: p.email, phone: p.phone }))
        if (action == 'view') {
