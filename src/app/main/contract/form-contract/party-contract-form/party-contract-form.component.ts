...
export class PartyContractFormComponent implements OnInit, AfterViewInit {
  ...
  selectWithOtp(e: any, data: any, type?: any) { // sort ordering
    ...
    if (type == 'organization') {
      //Nếu là người ký
      if (data.role == 3) {
        if (this.getDataSignHsm(data).length == 0 && this.getDataSignUSBToken(data).length == 0 && this.getDataSignEkyc(data.length) == 0 && this.getDataSignCert(data).length == 0 && this.getDataSignRemote(data).length == 0) {
          data.card_id = "";
        }
      }
      //Nếu là văn thư
  ...
  validData() {
    ...
    for (let i = 0; i < dataArr.length; i++) {
      ...
      if(!dataArr[i].card_id.trim() && (dataArr[i].role == 3 || dataArr[i].role == 4) &&
        (dataArr[i].sign_type.filter((p: any) => p.id == 2).length > 0 || dataArr[i].sign_type.filter((p: any) => p.id == 4).length > 0 || dataArr[i].sign_type.filter((p: any) => p.id == 6 || p.id == 8).length > 0)) {
    ...
    if (count == 0) {
      ...
      if (isOrderingPerson_exception.length > 0 && arrCompareOrderingPartner.length > 0) {
        ...
        let arrOrg_PersonPartner = this.datasForm.is_determine_clone.filter((val: any) => val.type != 2) // lấy ra mảng tổ chức của tôi và đối tác cá nhân

        let arrOrgPartner_PersonPartner = this.datasForm.is_determine_clone.filter((val: any) => val.type != 1) // lấy ra mảng tổ chức của đối tác và đối tác cá nhân
