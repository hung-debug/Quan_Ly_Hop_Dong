...
export class SampleContractFormComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.satisfiedFirstHandler = this.sysService.checkFirstHandler(this.datasForm.is_determine_clone)
  ...
  defindDataContract() {
    ...
    this.datasForm.is_determine_clone.forEach((res: any) => {
      res.recipients.forEach((element: any) => {
        let isObj = {
          ...
          role: element.role,
          template_recipient_id: element.template_recipient_id
        }
    ...
    if (dataDetermine.length > 0) {
      dataDiffirent = dataContractUserSign.filter((d: any) => !dataDetermine.some((data: any) =>
        (((d.sign_unit == 'chu_ky_anh' && data.sign_type.some((q: any) => q.id == 1 || q.id == 5) && (d.recipient_id == data.template_recipient_id || d.recipient_id == data.id)) ||
        ((d.sign_unit == 'text' && (data.sign_type.some((p: any) => p.id == 2 || p.id == 4 || p.id == 6))) && (d.recipient_id == data.template_recipient_id || d.recipient_id == data.id)) ||
        ((d.sign_unit == 'so_tai_lieu' && (data.sign_type.some((p: any) => p.id == 2 || p.id == 4 || p.id == 6))) && (d.recipient_id == data.template_recipient_id || d.recipient_id == data.id)) ||
        (d.sign_unit.includes('chu_ky_so') && data.sign_type.some((p: any) => p.id == 2 || p.id == 3 || p.id == 4 || p.id == 6 || p.id == 7 || p.id == 8) && (d.recipient_id == data.template_recipient_id || d.recipient_id == data.id)))
      ))
      || dataDetermine.some((data: any) => d.sign_unit.includes('chu_ky_so') && d.isSupportMultiSignatureBox == false && (d.recipient_id == data.template_recipient_id || d.recipient_id == data.id))
      );
    }
  ...
  validData(isSaveDraft?: any) {
    ...
    if (!data_not_drag) {
          ...
          if(
            ...
            (Number(coordinate_y[i]) <= (Number(coordinate_y[j]) + Number(height[j])) && (Number(coordinate_y[j] + Number(height[j])) <= (Number(coordinate_y[i]) + Number(height[i]))))
              ...
              if(!this.isCheckRelease && !isSaveDraft) {
                ...
                if (boxElements[i].sign_unit.includes('chu_ky_anh') && boxElements[j].sign_unit.includes('chu_ky_anh')) {
                  this.toastService.showErrorHTMLWithTimeout("Vị trí các ô ký ảnh không được để trùng hoặc giao nhau" + ` (trang ${boxElements[i].page})`,"",3000);
                }
                if (boxElements[i].sign_unit == boxElements[j].sign_unit && boxElements[i].sign_unit == "text" ) {
                  if ((boxElements[i].text_type == "default" && boxElements[i].text_type == boxElements[j].text_type) ||
                    (boxElements[i].type == boxElements[j].type && boxElements[i].type == 1) ||
      ...
      if (count > 0) {
        ...
        if(!this.isCheckRelease  && !isSaveDraft)
        this.toastService.showWarningHTMLWithTimeout(`Vui lòng nhập nội dung ô text/số tài liệu! (trang ${currentElement.page})`, "", 3000);
        return false;
      }
      else {
