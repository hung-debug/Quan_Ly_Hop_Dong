...
export class SampleContractComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  ...
  defindDataContract() {
    ...
    dataDetermine.forEach((data: any) => {
      dataContractUserSign.forEach((element: any) => {
        if (element.recipient_id == data.id ) {
          ...
          if (dataContractUserSign.filter((item: any) => item.recipient_id == data.id &&  [7,8].includes(data?.sign_type[0]?.id))?.length > 1) {
            element.isNotSupportTextField = true
          } else if (dataContractUserSign.filter((item: any) => item.recipient_id == data.id &&  [2,3,4,6].includes(data?.sign_type[0]?.id))?.length > 0) {
    ...
    //   || (val.sign_unit == 'chu_ky_so' && data.sign_type.some((p: any) => p.id == 2 || p.id == 3 || p.id == 4 || p.id == 6 || p.id == 7 || p.id == 8) && (val.recipient_id ? val.recipient_id as any : val.email as any) == (val.recipient_id ? data.id as any : data.email as any))
    // )
            ...
            res.type.forEach((resItem: any) => {
              if (resItem.sign_config.length > 0) {
                ...
                */
                ...
                resItem.sign_config = resItem.sign_config.filter((val: any) => !dataDiffirent.some((data: any) => !(val.recipient ? val.recipient : val.name) ||
        ...
        if(res.sign_unit == 'chu_ky_so') {
          res.type.forEach((element: any) => {
            element.sign_config.forEach((item: any) => {
              ...
              let change = dataDetermine.filter((data: any) => (item.recipient_id as any) == (data.id as any));
  ...
  getCheckSignature(isSignType: any, listSelect?: string, recipient_id?: any) {
    ...
    this.list_sign_name.forEach((element: any) => {
      if (isSignType != 'text' && (element.fields && element.fields.length && element.fields.length > 0) && element.fields.some((field: any) => field.sign_unit == isSignType)) {
        ...
        if (isSignType != 'text' && this.convertToSignConfig().some((p: any) => ((element.email && p.email == element.email) || (element.id && p.recipient_id == element.id)) && (p.sign_unit == isSignType || p.sign_unit.includes('chu_ky_so')))) {
          if (isSignType == 'so_tai_lieu') {
            ...
            element.is_disable = !element.sign_type.some((p: any) => p.id == 2 || p.id == 3 && count < 15 || p.id == 4 || p.id == 6)
  ...
  async onCancel(e: any, data: any) {
    ...
    if (data.id_have_data) {
      ...
      await this.contractTemplateService.deleteInfoContractSignature(data.id_have_data).toPromise().then((res: any) => {
        this.toastService.showSuccessHTMLWithTimeout(`Bạn đã xóa đối tượng ký trong tài liệu!`, "", "3000");
  ...
  getCheckDuplicateNameText() {
    ...
    for (let i = 0; i < this.datas.contract_user_sign.length; i++) {
      if (this.datas.contract_user_sign[i].sign_config.length > 0) {
        for (let j = 0; j < this.datas.contract_user_sign[i].sign_config.length; j++) {
          ...
          //neu la text thi neu khong duoc gan nguoi thi khong duoc trung nhau - fixed
          //form them moi: xet name null, form sua: xet recipient_id null - fixed
    ...
    for (var k = 0; k < arrCheckName.length; ++k) {
      ...
      if (value in valueSoFar) {
        this.dataTextDuplicate = this.dataTextDuplicate.filter((val: any) => val.value == value).map((item: any) => item.page.toString())
  ...
  validData() {
    ...
    if (!data_not_drag) {
      ...
      if (count > 0) {
        ...
        this.spinner.hide();
        this.toastService.showErrorHTMLWithTimeout(`Vui lòng chọn người ký cho đối tượng đã kéo thả! (trang ${currentElement.page})`, "", 3000);
        return false;
