...
export class ConsiderContractComponent
    ...
    if (
      ...
      if(this.firstHandler && !this.mobile) {
        this.datas.is_data_object_signature.map((item: any) => {
          if(item.type != 2 && item.type != 3 && item.type != 4 && (item.value && !item.recipient_id || item.recipient_id) && item.action_in_contract == false || (item.type == 4 && item.recipient_id && item.action_in_contract == false)) {
        ...
        if(this.markImage == true){
          ...
          if (!checkSign || (checkSign && !checkSign.success)) {
            ...
            try {
              ...
              let data = {
                eventName: pkiSupplierName, // Sử dụng tên nhà cung cấp PKI
                params: {
  ...
  validateSignature() {
    ...
    if(this.firstHandler) {
      this.isDataObjectSignature.map((item: any) => {
        if(item.type != 2 && item.type != 3 && item.type != 4 && item.value && !item.valueSign && !item.recipient_id) {
          validSign.push(item);
        }
      })
    }
