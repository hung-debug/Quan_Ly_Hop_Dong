...
export class DigitalCertificateService {
  ...
  deleteUserCTS(id_cert: any, id_customer: any) {
    this.getCurrentUser();
    const headers = new HttpHeaders()
      .append('Content-Type', 'application/json')
      .append('Authorization', 'Bearer ' + this.token);
  ...
  public searchCertificate(subject: string, serial_number: string, status: any, keystoreDateStart: any, keystoreDateEnd: any, number: any, size: any,): Observable<any> {
    this.getCurrentUser();
    if (keystoreDateStart != "") {
      keystoreDateStart = this.datepipe.transform(keystoreDateStart, 'yyyy-MM-dd');
    }
    ...
    const headers = new HttpHeaders()
      .append('Content-Type', 'application/json')
      .append('Authorization', 'Bearer ' + this.token);
      let listCertificate = this.getAllCert + '?subject=' + ( subject ? subject.trim() : '')+ '&serial_number=' + (serial_number ? serial_number.trim(): '') + '&status=' + status + '&size=' + size + '&number=' + number;
    return this.http.get<any>(listCertificate, { headers });
  }
